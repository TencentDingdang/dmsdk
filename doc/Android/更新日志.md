...
该版本开始不再分发AAR文件，请参考[接入指南](接入指南.md)直接从Maven Central（或JCenter）添加依赖。

该版本开始需要您手动接入微信和QQ登录的相关配置，包括Manifest配置和Gradle依赖，请参考[接入指南](接入指南.md)完成配置。
...
同时新增了[第三方账号方案的接入文档](接入指南.md#3-接入第三方账号方案)。
